...
1. JDK 설치
   ...
   - 설치위치: C:\Java\jdk
               C:\Java\jre
   - 환경설정: JAVA_HOME : C:\Java\jdk      
   - path: %JAVA_HOME%bin
   - download:  http://bit.ly/1lO1FSV
   
2. Scala 설치
   - version: 2.12.2
   - file name: scala-2.12.2.msi
   - 설치위치: C:\scala
   - 환경설정: SCALA_HOME : C:\scala
   - path: %SCALA_HOME%bin
   - download:  http://bit.ly/1hMUJIQ

3. VC++ 2010 SP1 Redistrivutable Package (x64)
   - download path: http://bit.ly/1ArYJpK
   - file name: vcredist_x64.exe

4. Hadoop
   - version: 2.7.1
   - file name: hadoop-2.7.1.tar.gz
   - 설치위치: C:\hadoop
   - 환경설정: HADOOP_HOME : C:\hadoop
   - path: %HADOOP_HOME%bin
   - download: http://bit.ly/1Xem6vr

5. Spark 설치
   // - version: 2.5.1
   // - file name: spark-1.5.1-bin-hadoop2.6.tgz
   // - download: http://bit.ly/1MOxf2M
   - version: 2.6.1
   - file name: spark-1.6.1-bin-hadoop2.6.tgz
   - 설치위치: C:\spark
   - 환경설정: SPARK_HOME : C:\spark
   - path: %HADOOP_HOME%bin
...
6. SCALA IDE
   - download: http://scala-ide.org/
   - file name: scala-SDK-4.6.1-vfinal-2.12-win32.win32.x86_64.zip
   
7. 아래 깃 허브에서 순서대로 신행한다.    
...
hadoop fs -cat /user/spark/item.txt
...
-------------------------------------------------------------------------------------------------------
val products = sc.textFile("/user/spark/item.txt")
val productsPair = products.map(line => line.split(',')).map(fields => (fields(0), fields(1)))
productsPair.collect
-----------------------------------------------------------------------------------------------------
val purchaseFlat = sc.textFile("/user/spark/purchase.txt").map(line => line.split(' ')).map(fields => (fields(0), fields(1))).flatMapValues(prod=> prod.split(';'))
val user = sc.textFile("/user/spark/user.txt").map(line => line.split(' ')).map(fields => (fields(0), fields(1)))
val result = purchaseFlat.join(user)
result.collect
--------------------------------------------------------------------------------------------------------------
val items = sc.textFile("/user/spark/item.txt").map(_.split(',')).map(f => (f(0),f(1)))
items.collect

var purchaseCnt = sc.textFile("/user/spark/web.log").
...
val proverb = sc.textFile("/user/spark/proverbs.txt")
...
------------------------------------------------------------------------------------
sc.textFile("/user/spark/wordcount.txt", 4).
...
groupByKey(2).
map(v => (v._1, v._2.sum / v._2.size.toDouble)).
saveAsTextFile("/user/spark/result")

hadoop fs -ls /user/spark/result
hadoop fs -cat /user/spark/result/*
------------------------------------------------------------------------------
val itemName = sc.textFile("/user/spark/item.txt").map(_.split(',')).map(f => (f(0),f(1)))

var purchaseCnt = sc.textFile("/user/spark/web.log",5).
filter(line => line.contains("action=purchase")).
map(line => line.split("&")(1)).
map(line => (line.split("=")(1), 1)).
reduceByKey((v1, v2) => v1 + v2)
...
object WeblogCounter {
  def main(args: Array[String]) {
    ...
    // spark 소스 시작
    //////////////////////////////////////////////////////////////////////
    var item = sc.textFile("/user/spark/item.txt")
    val itemName = item.map(_.split(',')).map(f => (f(0),f(1)))
  	
    val weblog = sc.textFile("/user/spark/web.log")
    ...
    result.saveAsObjectFile("/user/spark/item_count")
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
// Run As 에서 Maven build > Goals: package 넣고 > Run > 많은 것을 download 받고 > BUILD SUCCESS 
